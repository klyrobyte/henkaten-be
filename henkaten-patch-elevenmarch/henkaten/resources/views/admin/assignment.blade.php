...
<style>
/* ══ WRAP ══════════════════════════════════════════════════════ */
.assign-wrap { padding: 10px 12px 110px; }

/* ── Summary bar ─────────────────────────────────────────────── */
.sum-bar { display:grid; grid-template-columns:repeat(5,1fr); gap:8px; margin-bottom:14px; }
.sum-card { background:#fff; border-radius:10px; padding:12px 8px; text-align:center;
            box-shadow:0 2px 8px rgba(0,0,0,.07); border-top:3px solid var(--green-light); }
.sum-num  { font-family:'Orbitron',sans-serif; font-size:22px; font-weight:800; line-height:1; margin-bottom:3px; }
.sum-lbl  { font-size:9px; text-transform:uppercase; letter-spacing:.5px; color:#888; font-weight:700; font-family:'Roboto Condensed',sans-serif; }
.sc-hadir .sum-num { color:var(--green); }  .sc-hadir  { border-top-color:var(--green); }
.sc-absen .sum-num { color:var(--red); }    .sc-absen  { border-top-color:var(--red); }
.sc-butuh .sum-num { color:var(--yellow); } .sc-butuh  { border-top-color:var(--yellow); }
.sc-terisi .sum-num{ color:var(--navy); }   .sc-terisi { border-top-color:var(--navy); }
.sc-kosong .sum-num{ color:var(--orange); } .sc-kosong { border-top-color:var(--orange); }

/* ── Alert ───────────────────────────────────────────────────── */
.alert-strip { background:linear-gradient(90deg,var(--red),#c0392b); color:#fff;
...
.alert-strip .ai { font-size:18px; animation:apulse 1.5s infinite; }
@keyframes apulse { 0%,100%{transform:scale(1)} 50%{transform:scale(1.2)} }

/* ── Group block ─────────────────────────────────────────────── */
.group-block { background:#fff; border-radius:12px; box-shadow:0 2px 10px rgba(0,0,0,.07);
               border:2px solid var(--green-light); margin-bottom:14px; overflow:hidden; }
.group-hdr   { background:var(--green); color:#fff; padding:10px 14px;
               font-family:'Roboto Condensed',sans-serif; font-size:13px; font-weight:800;
               letter-spacing:.8px; text-transform:uppercase; display:flex; align-items:center; gap:8px; }
.group-hdr.kp{ background:linear-gradient(135deg,var(--navy),var(--blue)); }
.g-badge     { background:rgba(255,255,255,.2); padding:2px 8px; border-radius:8px; font-size:10px; font-weight:600; }
.g-badge.warn{ background:var(--orange); }
...
/* ══ ID CARD grid ════════════════════════════════════════════════
   ...
   └──────────────────┘
   ════════════════════════════════════════════════════════════════ */
.machine-grid { padding:12px; display:grid;
                grid-template-columns:repeat(auto-fill,minmax(128px,1fr));
                gap:10px; }

.mc-card { border-radius:12px; border:2px solid #dde8c8; background:#fff;
           box-shadow:0 2px 8px rgba(0,0,0,.07); overflow:hidden;
           transition:box-shadow .2s, border-color .2s;
           display:flex; flex-direction:column; }
.mc-card.card-ok      { border-color:#729E3F; }
.mc-card.card-partial { border-color:var(--yellow); box-shadow:0 2px 12px rgba(243,156,18,.2); }
.mc-card.card-problem { border-color:var(--red);    box-shadow:0 2px 16px rgba(231,76,60,.25); }

/* Card header */
.mc-card-hdr { padding:7px 9px 6px; display:flex; align-items:center; gap:6px;
               background:linear-gradient(135deg,#f0f5e8,#e5efd6);
               border-bottom:1px solid #d8e8c0; }
.mc-card.card-partial .mc-card-hdr { background:linear-gradient(135deg,#fffbe6,#fef3c0); border-bottom-color:#f0d060; }
.mc-card.card-problem .mc-card-hdr { background:linear-gradient(135deg,#fdeaea,#fad0d0); border-bottom-color:#f4a8a8; }

/* ── Status dot — DOT MERAH BISA DIKLIK ──────────────────────── */
.mc-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0;
          border:1.5px solid rgba(0,0,0,.1); transition:transform .15s; }
...
.dot-warn:hover, .dot-ab:hover { transform:scale(1.6); }
.dot-warn:active,.dot-ab:active{ transform:scale(.9); }
@keyframes abpulse   { 0%,100%{box-shadow:0 0 0 2px rgba(231,76,60,.4)}  50%{box-shadow:0 0 0 7px rgba(231,76,60,.0)} }
@keyframes warnpulse { 0%,100%{box-shadow:0 0 0 2px rgba(243,156,18,.4)} 50%{box-shadow:0 0 0 6px rgba(243,156,18,.0)} }

.mc-card-title { font-family:'Roboto Condensed',sans-serif; font-size:10px; font-weight:800;
                 color:#3a4a20; flex:1; white-space:nowrap; overflow:hidden;
                 text-overflow:ellipsis; text-transform:uppercase; letter-spacing:.3px; }
.mc-card.card-problem .mc-card-title { color:#c0392b; }
.mc-card.card-partial .mc-card-title { color:#8a6200; }

/* Card body */
.mc-card-body { padding:8px; display:flex; flex-direction:column; gap:6px; flex:1; }

/* Empty slot */
.mc-slot-empty { display:flex; align-items:center; justify-content:center; gap:3px;
                 border:2px dashed #ccc; border-radius:8px; padding:10px 4px;
                 color:#bbb; font-size:11px; font-family:'Roboto Condensed',sans-serif;
                 font-weight:700; cursor:pointer; transition:all .15s; min-height:60px; }
.mc-slot-empty:hover { border-color:var(--orange); color:var(--orange); background:rgba(255,128,0,.04); }

/* Member slot */
.mc-member { display:flex; flex-direction:column; align-items:center; gap:4px;
             border-radius:10px; padding:8px 6px 7px; position:relative;
             border:1.5px solid #dde8c8; background:#f8faf5; transition:all .15s; }
.ms-present { border-color:#b4d87a; background:#f4f9ec; }
.ms-absent  { border-color:#f5aaaa; background:#fdf0f0; }
.ms-sub     { border-color:var(--orange); background:#fff7ee; border-style:dashed; }

/* Avatar */
.mc-av { width:50px; height:50px; border-radius:50%; display:flex; align-items:center;
...
.av-present { background:linear-gradient(135deg,#8bc34a,var(--green)); }
.av-absent  { background:linear-gradient(135deg,#ef9a9a,#c0392b); filter:grayscale(.3); }
.av-sub     { background:linear-gradient(135deg,#ffb74d,var(--orange)); }

/* Name */
.mc-member-name { font-family:'Roboto Condensed',sans-serif; font-size:11px; font-weight:800;
                  text-align:center; line-height:1.2; color:var(--gray);
                  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
                  max-width:112px; width:100%; }
.ms-absent  .mc-member-name { color:#c0392b; }
.ms-sub     .mc-member-name { color:#c05800; }

/* Tag */
.mc-tag { font-size:8px; font-family:'Roboto Condensed',sans-serif; font-weight:800;
          text-transform:uppercase; padding:2px 6px; border-radius:4px; letter-spacing:.3px; }
.tag-hadir { background:var(--green);  color:#fff; }
.tag-absen { background:var(--red);    color:#fff; }
.tag-sub   { background:var(--orange); color:#fff; }
.tag-sync  { background:var(--navy);   color:#fff; }

/* Action buttons */
.mc-actions { display:flex; gap:3px; margin-top:2px; width:100%; }
.mc-btn { padding:4px 6px; border-radius:6px; border:none; cursor:pointer;
          font-size:9px; font-family:'Roboto Condensed',sans-serif; font-weight:800;
          transition:all .15s; flex:1; text-align:center; white-space:nowrap; }
.mc-btn:active { transform:scale(.9); }
.btn-ab   { background:rgba(231,76,60,.1);  color:var(--red); }
.btn-pr   { background:rgba(46,125,50,.1);  color:var(--green); }
.btn-find { background:var(--orange); color:#fff; padding:4px 8px; font-size:12px; flex:0 0 auto; }
.btn-rm   { background:rgba(231,76,60,.1); color:var(--red); }

/* ── Two-pane + Sub panel ────────────────────────────────────── */
.assign-two { display:grid; grid-template-columns:1fr 290px; gap:14px; align-items:start; }
@media(max-width:1100px){ .assign-two { grid-template-columns:1fr; } .sub-panel { position:static; max-height:none; } }
@media(max-width:600px) { .sum-bar { grid-template-columns:repeat(3,1fr); } }

.sub-panel { background:#fff; border-radius:12px; box-shadow:0 2px 10px rgba(0,0,0,.08);
             border:2px solid var(--green-light); overflow:hidden;
             position:sticky; top:80px; max-height:calc(100vh - 100px);
             display:flex; flex-direction:column; }
.sub-ph    { background:linear-gradient(135deg,var(--orange),#d46200); padding:14px 16px; color:#fff; }
.sub-ph h2 { font-family:'Orbitron',sans-serif; font-size:13px; font-weight:900; margin:0 0 2px; }
.sub-ph p  { font-size:11px; opacity:.85; margin:0; }
.sub-ctx   { background:rgba(0,0,0,.2); border-radius:6px; padding:5px 8px; margin-top:8px;
             font-family:'Roboto Condensed',sans-serif; font-size:11px; font-weight:700; display:none; }
.sub-ctx.show { display:block; }

.sub-idle  { padding:28px 14px; text-align:center; color:#bbb; }
.idle-ico  { font-size:40px; margin-bottom:8px; }
.sub-idle h3 { font-family:'Orbitron',sans-serif; font-size:12px; font-weight:800; color:#ccc; margin-bottom:6px; }
.sub-idle p  { font-size:11px; line-height:1.7; color:#aaa; }

.sub-search-wrap { padding:10px 12px; border-bottom:1px solid #e0ecd4; }
.sub-search-wrap input { width:100%; padding:8px 10px; border:2px solid var(--green-light);
                         border-radius:8px; font-size:13px; box-sizing:border-box; }
.sub-search-wrap input:focus { outline:none; border-color:var(--green); }

.cand-list { overflow-y:auto; padding:8px; display:grid; grid-template-columns:1fr 1fr; gap:7px; flex:1; }
.cand-list::-webkit-scrollbar { width:4px; }
.cand-list::-webkit-scrollbar-thumb { background:var(--green); border-radius:2px; }

.cand-card { display:flex; flex-direction:column; align-items:center; gap:5px;
             padding:10px 5px 8px; border-radius:10px; border:2px solid var(--green-light);
             background:#f8faf5; cursor:pointer; transition:all .18s; text-align:center; }
.cand-card:active { border-color:var(--orange); background:rgba(255,128,0,.05); transform:scale(.96); }
.cand-av   { width:44px; height:44px; border-radius:50%; display:flex; align-items:center;
...
.av-n { background:linear-gradient(135deg,var(--green-light),var(--green)); }
.av-w { background:linear-gradient(135deg,#f39c12,#e67e22); border-color:var(--yellow); }
.cand-name { font-family:'Roboto Condensed',sans-serif; font-size:11px; font-weight:800; line-height:1.2; }
.cand-tag  { font-size:8px; font-family:'Roboto Condensed',sans-serif; font-weight:700;
             padding:2px 5px; border-radius:4px; border:1px solid var(--yellow);
             color:var(--yellow); background:rgba(243,156,18,.1); }
.cand-btn  { width:100%; padding:5px 0; border-radius:6px; border:none;
             background:var(--orange); color:#fff; font-family:'Roboto Condensed',sans-serif;
             font-weight:900; font-size:11px; cursor:pointer; }
.cand-empty { text-align:center; padding:20px 10px; color:#aaa; grid-column:1/-1; font-size:12px; }

/* ── Sync strip ──────────────────────────────────────────────── */
.sync-strip { background:linear-gradient(90deg,#1565C0,var(--navy)); color:#fff; padding:7px 14px;
              border-radius:8px; margin-bottom:10px; display:none; align-items:center; gap:8px;
              font-family:'Roboto Condensed',sans-serif; font-size:12px; font-weight:600; }
.sync-strip.show { display:flex; }

/* ── Save bar ────────────────────────────────────────────────── */
.assign-save-bar { position:fixed; bottom:56px; left:0; right:0; background:var(--navy);
                   padding:10px 18px; display:none; align-items:center; gap:12px; z-index:200;
                   box-shadow:0 -4px 18px rgba(0,0,0,.2); border-top:3px solid var(--orange); }
.assign-save-bar.show { display:flex; }
.asb-msg  { color:rgba(255,255,255,.85); font-size:13px; font-weight:600; flex:1; }
.asb-msg strong { color:var(--orange); }
.btn-asb-save    { padding:8px 20px; border-radius:8px; border:none; background:var(--green);
                   color:#fff; font-weight:800; font-size:13px; cursor:pointer; }
.btn-asb-discard { padding:8px 14px; border-radius:8px; border:1.5px solid rgba(255,255,255,.3);
                   background:transparent; color:rgba(255,255,255,.7); font-size:12px; cursor:pointer; }

/* ── Modal alasan absen ─────────────────────────────────────── */
.modal-bg2 { position:fixed; inset:0; background:rgba(0,0,0,.55); z-index:600;
             display:flex; align-items:center; justify-content:center; padding:16px;
             opacity:0; pointer-events:none; transition:opacity .2s; }
.modal-bg2.open { opacity:1; pointer-events:all; }
.modal-box2 { background:#fff; border-radius:14px; width:100%; max-width:360px;
              transform:scale(.93) translateY(18px); transition:transform .22s; overflow:hidden; }
.modal-bg2.open .modal-box2 { transform:scale(1) translateY(0); }
.modal-hdr2 { background:linear-gradient(135deg,var(--green),var(--navy)); padding:14px 18px;
              display:flex; align-items:center; justify-content:space-between; }
.modal-hdr2 h3 { font-family:'Orbitron',sans-serif; font-size:13px; font-weight:900; color:#fff; margin:0; }
.modal-hdr2 button { background:transparent; border:1.5px solid rgba(255,255,255,.35);
                     color:#fff; width:26px; height:26px; border-radius:50%; font-size:16px; cursor:pointer; }
.reason-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; padding:16px 18px; }
.reason-btn  { padding:11px 8px; border-radius:8px; border:2px solid var(--green-light);
               background:#f8faf5; font-family:'Roboto Condensed',sans-serif; font-size:13px;
               font-weight:700; cursor:pointer; transition:all .15s; }
.reason-btn.sel { background:var(--red); color:#fff; border-color:var(--red); }
.modal-foot2 { padding:0 18px 16px; display:flex; gap:8px; justify-content:flex-end; }
.btn-cancel  { padding:9px 18px; border-radius:8px; border:2px solid #e0e0e0; background:#f5f5f5;
               font-weight:700; font-size:13px; cursor:pointer; }
.btn-confirm { padding:9px 18px; border-radius:8px; border:none; background:var(--red);
...
<div class="legend-4m">
    ...
    <div style="margin-left:auto;font-size:10px;color:#888">💡 Klik dot merah untuk cari pengganti</div>
</div>
...
<div class="assign-wrap">
    ...
    {{-- Summary --}}
    <div class="sum-bar">
        <div class="sum-card sc-hadir"><div class="sum-num" id="sumHadir">{{ $summary['hadir'] }}</div><div class="sum-lbl">Hadir</div></div>
        <div class="sum-card sc-absen"><div class="sum-num" id="sumAbsen">{{ $summary['absen'] }}</div><div class="sum-lbl">Absen</div></div>
        <div class="sum-card sc-butuh"><div class="sum-num" id="sumButuh">{{ $summary['butuh'] }}</div><div class="sum-lbl">Butuh Pengganti</div></div>
        <div class="sum-card sc-terisi"><div class="sum-num" id="sumTerisi">{{ $summary['terisi'] }}</div><div class="sum-lbl">Terisi</div></div>
        <div class="sum-card sc-kosong"><div class="sum-num" id="sumKosong">{{ $summary['kosong'] }}</div><div class="sum-lbl">Kosong</div></div>
    </div>
        ...
        <div>
            <div class="sub-panel" id="subPanel">
                ...
                <div class="sub-idle" id="subIdle">
                    ...
                    <p>Tandai member absen,<br>lalu klik <strong style="color:var(--red)">dot merah</strong><br>atau tap <strong style="color:var(--orange)">🔍</strong>.</p>
                </div>
