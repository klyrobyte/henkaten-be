...
<style>
/* ══ MACHINE SECTION ══════════════════════════════════════════════ */
.machines-wrap { padding: 0 0 110px; }
...
.mc-photo-wrap {
    ...
    overflow: hidden; border-bottom: 1px solid #e0ecd4;
}
...
.mc-photo-wrap img { width: 100%; height: 100%; object-fit: cover; transition: transform .3s; }
...
.mc-photo-wrap:hover .mc-photo-upload-overlay { background: rgba(0,0,0,.45); pointer-events: all; }
...
.mc-photo-upload-overlay .upload-txt {
    ...
    transition: opacity .2s .05s, transform .2s .05s; pointer-events: none;
}
.mc-photo-wrap:hover .upload-ico,
.mc-photo-wrap:hover .upload-txt { opacity: 1; transform: translateY(0); }
.mc-photo-wrap.uploading .mc-photo-upload-overlay { background: rgba(0,0,0,.55); pointer-events: none; }
...
.mc-name-badge .mc-4m-row { pointer-events: all; }
.mc-name-txt {
    font-family: 'Roboto Condensed', sans-serif;
    font-size: 11px; font-weight: 800; color: #fff;
    text-transform: uppercase; letter-spacing: .4px;
    text-shadow: 0 1px 3px rgba(0,0,0,.5); line-height: 1;
}
.mc-dot {
    width: 10px; height: 10px; border-radius: 50%;
    border: 1.5px solid rgba(255,255,255,.6);
    display: none; flex-shrink: 0; transition: transform .15s;
}
.mc-dot.d-visible { display: block; }
.mc-dot.d-absen {
    display: block; background: #e74c3c; cursor: pointer; pointer-events: all;
...
.mc-addlog-bar { display: flex; justify-content: center; padding: 4px 0 2px; }
.mc-addlog-btn {
    display: flex; align-items: center; gap: 3px;
    ...
    font-family: 'Roboto Condensed', sans-serif;
    font-size: 9px; font-weight: 800; text-transform: uppercase;
    letter-spacing: .4px; color: #bbb; cursor: pointer; transition: all .15s;
}
.mc-addlog-btn:hover {
    border-color: var(--orange, #e65100); color: var(--orange, #e65100);
    background: rgba(230,81,0,.06); transform: scale(1.05);
}
...
.finder-overlay {
    ...
    z-index: 1100; display: none; align-items: flex-end; justify-content: center;
}
...
.finder-handle { width: 44px; height: 5px; background: #ddd; border-radius: 3px; margin: 12px auto 0; flex-shrink: 0; }
.finder-ph {
    background: linear-gradient(135deg, #e74c3c, #c0392b);
    padding: 14px 18px 16px; color: #fff; flex-shrink: 0; position: relative;
}
.finder-ph h2 { font-family: 'Orbitron', sans-serif; font-size: 13px; font-weight: 900; margin: 0 0 2px; }
...
.finder-search-wrap { padding: 12px 14px; border-bottom: 1px solid #eee; flex-shrink: 0; }
...
.cand-empty { text-align: center; padding: 24px 10px; color: #aaa; grid-column: 1/-1; font-size: 12px; }

#qlLogList { max-height: 230px; overflow-y: auto; margin-bottom: 4px; }
.ql-log-item { border-radius: 10px; border: 1.5px solid #eee; padding: 8px 10px; margin-bottom: 6px; background: #fafafa; }
.ql-log-open   { border-left: 3px solid #e74c3c; }
.ql-log-closed { border-left: 3px solid #4caf50; opacity: .75; }
.ql-log-top { display: flex; align-items: center; gap: 5px; margin-bottom: 4px; flex-wrap: wrap; }
.ql-log-badge {
    font-family: 'Roboto Condensed', sans-serif; font-size: 9px; font-weight: 900;
    padding: 2px 7px; border-radius: 5px; color: #fff;
    text-transform: uppercase; letter-spacing: .4px;
}
.ql-log-dot  { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
.dot-open    { background: #e74c3c; animation: pulse-ql 1.5s infinite; }
.dot-closed  { background: #4caf50; }
@keyframes pulse-ql {
    0%,100% { box-shadow: 0 0 0 0 rgba(231,76,60,.4); }
    50%      { box-shadow: 0 0 0 4px rgba(231,76,60,0); }
}
.ql-log-time { font-family:'Roboto Condensed',sans-serif; font-size:10px; color:#888; font-weight:600; }
.ql-log-dur  { font-family:'Roboto Condensed',sans-serif; font-size:10px; color:#aaa; margin-left:auto; }
.ql-log-dur.blink { color:#e74c3c; font-weight:800; animation:blink-ql .9s step-end infinite; }
@keyframes blink-ql { 0%,100%{opacity:1} 50%{opacity:.3} }
.ql-log-desc { font-size:11px; color:#555; line-height:1.4; margin-bottom:5px; }
.ql-log-meta { font-size:10px; color:#888; margin-bottom:2px; }
.ql-log-actions { display:flex; gap:5px; }
.ql-btn {
    padding: 4px 10px; border-radius: 7px; border: 1.5px solid #ddd;
    background: #f5f5f5; font-size: 10px; font-weight: 800;
    font-family: 'Roboto Condensed', sans-serif; cursor: pointer; transition: all .15s;
}
.ql-btn-done   { border-color:#a5d6a7; color:#2e7d32; background:#f1f8e9; }
.ql-btn-done:hover   { background:#c8e6c9; }
.ql-btn-reopen { border-color:#90caf9; color:#1565c0; background:#e3f2fd; }
.ql-btn-reopen:hover { background:#bbdefb; }
.ql-btn-del    { border-color:#ffcdd2; color:#c62828; background:#fff5f5; margin-left:auto; }
.ql-btn-del:hover    { background:#ffcdd2; }
...
.ql-form-section { display:none; animation:qlFadeIn .2s; }
.ql-form-section.visible { display:block; }
@keyframes qlFadeIn { from{opacity:0;transform:translateY(4px)} to{opacity:1;transform:none} }
...
.ql-shb-machine  { background:#eef1fa; color:#1f3c88; border:1.5px solid #c5cae9; }
.ql-shb-material { background:#fff8ec; color:#e67e22; border:1.5px solid #ffe0b2; }
.ql-shb-method   { background:#f1f8e9; color:#2e7d32; border:1.5px solid #c8e6c9; }
...
.ql-form-divider::before,.ql-form-divider::after { content:''; flex:1; height:1px; background:#eee; }

#qlSaveBtnWrap { display:none; }
.save-btn-big {
...
.save-btn-big:active { opacity: .85; transform: scale(.98); }
.save-btn-big:disabled { opacity: .55; cursor: not-allowed; }
</style>
@endpush
...
<div class="machines-wrap">
    ...
    @php
        ...
        foreach($group['machines'] as $mac) {
            ...
            if ($macHasAbsen && !$macHasReplacement) $absenMesinCount++;
        }
    @endphp
        ...
        <div class="machine-cards-row">
            @foreach($group['machines'] as $machine)
                @php
                    ...
                    foreach ($stAll as $s) {
                        if ($s !== 'man' && !in_array($s, $initPips)) $initPips[] = $s;
                    }
                    ...
                    foreach ($stAll as $s) {
                        if (!in_array($s, $activePills)) $activePills[] = $s;
                    }
                        ...
                        <div class="mc-name-badge">
                            ...
                            <div style="display:flex;align-items:center;gap:4px">
                                ...
                                <div class="mc-dot {{ $dotInitClass }}"
                                     id="dot-{{ $machineSlug }}"
                                     @if($dotInitBg) style="background:{{ $dotInitBg }}" @endif
                                     @if($hasAbsen && $needsFinder)
    ...
    <div class="modal-sheet" style="max-height:94vh;display:flex;flex-direction:column;">
        ...
        <div class="modal-sheet-body" style="padding-top:6px;overflow-y:auto;flex:1;min-height:0;">
            ...
            <div class="ql-form-section" id="ql-section-machine">
                ...
                <div class="form-row">
                    <div class="field-group"><label>Root Cause</label><input type="text" id="qlCause" placeholder="Contoh: bearing aus…"></div>
                    <div class="field-group"><label>Teknisi / PIC</label><input type="text" id="qlPIC" placeholder="Nama teknisi"></div>
                </div>
            </div>
            ...
            <div class="ql-form-section" id="ql-section-material">
                ...
                <div class="form-row">
                    <div class="field-group"><label>No. Lot / Batch</label><input type="text" id="ql-mat-cause" placeholder="No. lot material bermasalah"></div>
                    <div class="field-group"><label>PIC</label><input type="text" id="ql-mat-pic" placeholder="Nama penanggung jawab"></div>
                </div>
            </div>
            ...
            <div class="ql-form-section" id="ql-section-method">
                ...
                <div class="field-group">
                    ...
                    <textarea id="ql-met-deskripsi" rows="2" placeholder="Contoh: setting tidak sesuai standar, tidak mengikuti SOP…"
                        style="width:100%;padding:10px;border:1.5px solid #e0e0e0;border-radius:10px;font-family:inherit;font-size:13px;resize:vertical;box-sizing:border-box"
                        onfocus="this.style.borderColor='#2e7d32'" onblur="this.style.borderColor='#e0e0e0'"></textarea>
                </div>
                <div class="form-row">
                    <div class="field-group"><label>Standar yang Dilanggar</label><input type="text" id="ql-met-cause" placeholder="Contoh: suhu resin, cycle time SOP…"></div>
                    <div class="field-group"><label>PIC</label><input type="text" id="ql-met-pic" placeholder="Nama penanggung jawab"></div>
                </div>
            </div>
        </div>

        <div style="flex-shrink:0;padding:10px 16px 16px;border-top:1px solid #f0f0f0;background:#fff;" id="qlSaveBtnWrap">
...
async function syncCards() {
    ...
    try {
        ...
        // Map mesin → array jenis open log (bisa lebih dari satu)
        const openLogMap = {};
        ...
        document.querySelectorAll('.mc-card').forEach(card => {
            ...
            // Pip dots: man (jika ada absen) + semua open log
            ...
            // Primary status untuk border & dot
            ...
            if (cardHasAbsen) {
                ...
                dot.onclick = cardNeedsFinder ? (e=>{e.stopPropagation();openFinderModal(machine);}) : null;
...
async function pickCandidate(memberId,name,photo,sourceMachine){
    ...
    try {
        const res=await fetch('/admin/replacements',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF,'Accept':'application/json'},body:JSON.stringify({tanggal:TANGGAL,factory:FACTORY,shift:SHIFT,target_machine:activeMachine,member_id:memberId})});
    ...
    try {
        ...
        data.forEach(r=>{
            ...
            if(r.source_machine){
                ...
                if(sc){const mi=sc.querySelector(`.mc-member-item[data-member-id="${r.member_id}"]`);
...
function startAutoRefresh(){
    ...
    _arInterval=setInterval(()=>{
        ...
        if(_arCountdown<=0){_arCountdown=30;syncAll();}
    },1000);
}
function toggleAutoRefresh(){
    ...
    else{_arCountdown=30;dot?.classList.remove('paused');if(btn)btn.textContent='⏸ Pause';}
}
