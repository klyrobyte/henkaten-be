...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Orbitron:wght@700;900&family=Roboto+Condensed:wght@400;700;900&display=swap"
        rel="stylesheet">
    <style>
        * {
        ...
        .shift-btn:not(:last-child) {
            border-right: 1px solid #e8e8e8;
        }

        .factory-2 .shift-btn:hover {
            background: #e8f5e9;
        }

        .factory-34 .shift-btn:hover {
            background: #e3f2fd;
        }
...
<body>
    ...
    <div class="picker-grid">
        @foreach($factories as $fac)
            <div class="factory-card" style="border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.4);">
                <div class="factory-header" style="background:{{ $fac->gradient }};">
                    <span class="icon">🏭</span>
                    <span class="name">{{ $fac->name }}</span>
                </div>
                <div class="shift-row">
                    <a class="shift-btn" href="{{ route('admin.tv') }}?factory={{ urlencode($fac->name) }}&shift=A">
                        <span class="shift-factory-code">{{ $fac->short_label }}</span>
                        <span class="shift-badge badge-a">Shift A</span>
                    </a>
                    <a class="shift-btn" href="{{ route('admin.tv') }}?factory={{ urlencode($fac->name) }}&shift=B">
                        <span class="shift-factory-code">{{ $fac->short_label }}</span>
