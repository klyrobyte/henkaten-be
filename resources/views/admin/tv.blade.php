...
<head>
   ...
   ════════════════════════════════════════════════════════════════ */
        ...
        .tv-body-inner {
            /* content lives here — do not set transform directly */
        }

        /* Scroll track wraps [original + clone], gets the translateY */
        .tv-scroll-track {
    ...
    <div class="app-header" style="padding:0 12px;gap:10px;position:fixed;top:0;left:0;right:0;z-index:400;">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0;">
            ...
            <div style="display:flex;flex-direction:column;line-height:1.2;min-width:0;">
                <span
                    style="font-family:'Orbitron',sans-serif;font-weight:900;font-size:22px;letter-spacing:2px;color:#f5a623;text-shadow:0 0 14px rgba(245,166,35,.5);white-space:nowrap;">HENKATEN
                    BOARD</span>
                <span
    ...
    <div class="tv-body" id="tvBody">
        <div class="tv-body-inner" id="tvInner">
            ...
            <div class="date-bar">
                ...
                <span
                    style="font-family:'Roboto Condensed',sans-serif;font-size:16px;font-weight:700;color:#444;flex:1;">
                    {{ \Carbon\Carbon::parse($tanggal)->isoFormat('dddd, D MMMM YYYY') }}
                </span>
                <span
                    style="background:var(--green-light,#8bc34a);color:#fff;padding:5px 14px;border-radius:20px;font-family:'Roboto Condensed',sans-serif;font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;">
            ...
            <div class="status-panel">
                ...
                <div class="status-emot-row">
                    <div class="emot-card {{ $statusLevel === 0 ? 'active-green' : '' }}" id="ec-green"> <span
                            class="emot-icon">🟢</span>
                        <div class="emot-label">AMAN</div>
                    </div>
                    <div class="emot-card {{ $statusLevel === 1 ? 'active-yellowgreen' : '' }}" id="ec-yg"> <span
                            class="emot-icon">🟡</span>
                        <div class="emot-label">RINGAN</div>
                    </div>
                    <div class="emot-card {{ $statusLevel === 2 ? 'active-yellow' : '' }}" id="ec-yellow"><span
                            class="emot-icon">⚠️</span>
                        <div class="emot-label">KHUSUS</div>
                    </div>
                    <div class="emot-card {{ $statusLevel === 3 ? 'active-red' : '' }}" id="ec-red"> <span
                            class="emot-icon">🔴</span>
            ...
            <div class="summary-grid">
                ...
                <div class="summary-card">
                    <div class="s-val" id="sTotalMP">{{ $absenceSummary?->total_member ?? 0 }}</div>
                ...
                <div class="summary-card" style="border-bottom-color:var(--yellow)">
                    <div class="s-val" id="sMaterial" style="color:var(--yellow)">{{ $machineSummary['material'] }}
                    </div>
                    <div class="s-lbl">Material</div>
                </div>
                <div class="summary-card" style="border-bottom-color:var(--green-light)">
                    <div class="s-val" id="sMethod" style="color:var(--green-light)">{{ $machineSummary['method'] }}
                    </div>
            ...
            <div class="attendance-chart-wrap">
                ...
                <div class="legend-grid">
                    ...
                    <div class="legend-item"><span class="leg-dot" style="background:#5dade2"></span>Pengawas Sakit
                    </div>
            ...
            <div class="legend-4m" style="margin-bottom:10px">
                ...
                <div class="legend-4m-item" style="color:#607d8b;font-weight:700;font-size:14px">▨ = tugas di mesin lain
                </div>
            </div>
                                                    ...
                                                    @if($replM)
                                                        ...
                                                        <div class="mc-member-item" data-member-id="{{ $replM->id }}" data-replacement="1">
                                                            <div class="mc-av av-repl">
                                                                @if($replM->photo_url)<img src="{{ $replM->photo_url }}"
                                                                    alt="{{ $replM->nama }}">
    ...
    <script>
        ...
        function renderChart(d) {
            ...
            tvChart = new Chart($g('myChart'), {
                type: 'doughnut',
                data: {
                    labels: ['MP Hadir', 'OP Cuti', 'OP Sakit', 'OP Ijin', 'Pengawas Cuti', 'Pengawas Sakit', 'Pengawas Ijin'],
        ...
        /* ── AUTO SCROLL — seamless infinite loop (no teleport, no pause) ── */
        const tvBody   = $g('tvBody');
        const tvInner  = $g('tvInner');   // the original content block
        const tvPBadge = $g('tvPaused');
        let posY = 0, paused = false, ptmr = null;
        const SPD = 0.55;                 // px per animation frame (~33 px/s at 60 fps)

        /* ── Build scroll track: [original | clone] ──────────────────────
           Wrap tvInner in a .tv-scroll-track div, then append a clone of
           tvInner as the second child.  We translateY the TRACK so both
           copies scroll together.  When posY reaches the height of the
           original copy we subtract that height — visually identical frame,
           zero teleport, zero pause.                                       */
        const track = document.createElement('div');
        track.className = 'tv-scroll-track';
        tvBody.insertBefore(track, tvInner);  // insert track before inner
        track.appendChild(tvInner);           // move inner into track
        const clone = tvInner.cloneNode(true);
        clone.setAttribute('aria-hidden', 'true');
        clone.removeAttribute('id');
        track.appendChild(clone);             // [original][clone] side-by-side vertically
        ...
        function raf() {
            if (!paused) {
                /* originalH = rendered height of the original copy only */
                const originalH = tvInner.offsetHeight;
                if (originalH > 0) {
                    posY += SPD;
                    /* Seamless reset: posY wraps back within [0, originalH) */
                    if (posY >= originalH) posY -= originalH;
                }
                applyScroll();
            }
        ...
        /* ── ORIGINAL mouse / touch / key handlers (unchanged) ─────────── */
        tvBody.addEventListener('mouseenter', () => { paused = true; tvPBadge.classList.add('show'); });
        tvBody.addEventListener('mouseleave', () => {
            /* Resume auto-scroll from wherever the user stopped — don't jump to top */
            tvPBadge.classList.remove('show');
            clearTimeout(ptmr);
            ptmr = setTimeout(() => { paused = false; }, 800);
        });
        ...
        /* ── NEW: Wheel-scroll inside the TV body ───────────────────────
           • Intercepts the native wheel event so the page itself never scrolls.
           • Moves posY by the wheel delta (clamped to valid range).
           • Auto-scroll stays paused while mouse is inside (mouseenter already
             set paused=true). When the mouse leaves, mouseleave resumes it from
             the new posY — so scroll picks up exactly where the user left off.
           ─────────────────────────────────────────────────────────────── */
        tvBody.addEventListener('wheel', e => {
            e.preventDefault();
            const originalH = tvInner.offsetHeight;
            if (originalH <= 0) return;
            const delta = e.deltaY || e.deltaX || 0;
            posY = (posY + delta * 0.6 + originalH) % originalH; // stay in [0, originalH)
            applyScroll();

            paused = true;
            tvPBadge.classList.add('show');
            clearTimeout(ptmr);
            ptmr = setTimeout(() => {
                if (paused) { paused = false; tvPBadge.classList.remove('show'); }
            }, 3000);
        }, { passive: false });
