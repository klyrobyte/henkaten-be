...
@push('styles')
    <style>
        ...
        .gl-filter-group input:focus,
        .gl-filter-group select:focus {
            outline: none;
            border-color: #0f3460;
        }
        ...
        .gl-pagination .page-link.active {
            background: #0f3460;
            color: #fff;
            border-color: #0f3460;
        }
...
@section('content')
    <div class="gl-wrap">
        ...
        {{-- Header --}}
        <div class="gl-header" style="display: none">
        ...
        <form method="GET" action="{{ route('admin.global-logs.index') }}" class="gl-filters" id="glFilterForm">
            ...
            <a href="{{ route('admin.global-logs.index') }}" class="btn-gl-filter"
                style="background:#607d8b;text-decoration:none;">✕ Reset</a>
            <a href="{{ route('admin.global-logs.export', request()->query()) }}" class="btn-gl-export" id="glExportBtn">⬇
                Export CSV</a>
        </form>
                    ...
                    <tbody>
                        @foreach($logs as $log)
                            ...
                            <tr>
                                ...
                                <td style="font-family:monospace;font-size:11px;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"
                                    title="{{ $log->target }}">{{ $log->target }}</td>
                                <td>{{ $log->factory ?? '-' }}</td>
                                <td style="font-family:monospace;font-size:11px">
                                    {{ \App\Models\GlobalLog::safeDecrypt($log->ip_enc) }}
                                </td>
                                <td
                                    style="font-weight:700;color:{{ str_starts_with((string) $statusCode, '2') ? '#2e7d32' : (str_starts_with((string) $statusCode, '4') ? '#c62828' : '#555') }}">
                                    {{ $statusCode }}
                                </td>
                            </tr>
                        @endforeach
        ...
        <p style="text-align:center;color:#aaa;font-size:11px;margin-top:12px">
            Total: {{ number_format($logs->total()) }} record — Halaman {{ $logs->currentPage() }} dari
            {{ $logs->lastPage() }}
        </p>
    </div>
@endsection
