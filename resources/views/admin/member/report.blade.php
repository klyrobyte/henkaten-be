...
@section('content')
    ...
    <div class="main">
        ...
        <div style="display:flex;gap:8px;margin-bottom:14px">
            ...
            <a href="{{ route('admin.absence.export', ['tanggal' => $tanggal]) }}" class="btn btn-sm btn-navy">📤 Export
                CSV</a>
        </div>
                    ...
                    @else
                        @foreach($r['absenList'] as $m)
                            ...
                            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f0f0f0">
                                <div
                                    style="width:32px;height:32px;border-radius:50%;overflow:hidden;background:#f0f0f0;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0">
                                    @if($m->photo_url)
                                ...
                                <span
                                    style="background:{{ $dotColors[$rec?->reason ?? ''] ?? '#888' }};color:#fff;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;flex-shrink:0">
                                    {{ $rec?->reason ?? '-' }}
                                </span>
                            </div>
                        @endforeach
