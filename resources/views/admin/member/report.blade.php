...
async function confirmCloseLog() {
    ...
    try {
        ...
        if (timeEl && waktuSelesai) {
            const mulai = timeEl.textContent.split('–')[0].trim();
            timeEl.textContent = `${mulai} – ${waktuSelesai}`;
        }
        if (durEl) {
        ...
        // Tampilkan / update CM di card body
...
async function reopenLog(id) {
    ...
    try {
        ...
        if (dotEl) {
            dotEl.className = 'log-status-dot dot-open';
        }
        if (timeEl) {
            // Hapus waktu selesai dari tampilan
            const mulai = timeEl.textContent.split('–')[0].trim();
